...
Checkmate comes with an official React integration that allows you to use Checkmate with React.
...
To use Checkmate with React, you need to create a store using the `createStore` function from the `@genshi/react` package.
...
A common use case is to pass a payload to an action or an effect. Checkmate comes with first-class support for payloads.
